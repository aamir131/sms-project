...
from smsapp.models import Students, Course, Subjects, CustomUser, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, StudentResult
...
def student_home(request):
    ...
    return render(request,'student_template/student_home_template.html',{"total_attendance":total_attendance,"present_attendance":present_attendance,"absent_attendance":absent_attendance,"subjects":subjects,"data_name":subject_name,"data1":present_data,"data2":absent_data})
...
def student_apply_leave_save(request):
    if request.method!="POST":
        ...
        student_obj=Students.objects.get(admin=request.user.id)
        try:
            leave_report=LeaveReportStudent(student_id=student_obj,leave_date=leave_date,leave_massege=leave_msg,leave_status=0)
            leave_report.save()

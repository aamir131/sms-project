...
from smsapp.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, LeaveReportStaff, Staffs, FeedBackStaff, CustomUser, Course, StudentResult
...
def staff_home(request):
    ...
    for student in students_attendance:
        ...
        student_list_attendance_present.append(present_attendance_count)
        student_list_attendance_absent.append(absent_attendance_count)
...
def staff_apply_leave_save(request):
    if request.method!="POST":
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            leave_report=LeaveReportStaff(staff_id=staff_obj,leave_date=leave_date,leave_massege=leave_msg,leave_status=0)
            leave_report.save()
...
def save_student_result(request):
    ...
    try:
        ...
        if check_exist:
            result=StudentResult.objects.get(subject_is=subject_obj,student_id=student_obj)
        ...
        messages.error(request,"Failed Added Result")
        return HttpResponseRedirect(reverse("staff_add_result"))        
